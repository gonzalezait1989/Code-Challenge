...
public class PokemonServiceTestImpl {
  ...
  public void prepareDataSet() {
    ...
    Pokemon bulbasaur = new Pokemon(1, "bulbasaur", count, count, false, count, count,
        new NamedApiResource("", "", 1), pokemonAbilities, results, gameIndices, heldItems, moves,
        stats, types, sprites);
    ...
    NamedApiResourceList namedResourceApiList = new NamedApiResourceList(count, next, previous,
        results);
    ...
    when(pokemonMessageProducer.produceFindByName(" ")).thenReturn(null);
    when(pokemonMessageProducer.produceFindByName("aitor"))
        .thenReturn(MessageBuilder.withPayload("aitor").build());
  ...
  public void testFindByNameAsync() {
    ...
    assertThat(pokemonService.findByNameAsync("bulbasaur").get().getPayload())
        .isEqualTo("bulbasaur");
  }
