...
public class PokemonServiceImplTest {
	...
	public void prepareDataSet() {
		...
		Pokemon bulbasaur = new Pokemon(1, "bulbasaur", count, count, false, count, count,
				new NamedApiResource("", "", 1), pokemonAbilities, results, gameIndices, heldItems, moves, stats, types,
				sprites);
		Pokemon charmander = new Pokemon(4, "charmander", count, count, false, count, count,
				new NamedApiResource("", "", 1), pokemonAbilities, results, gameIndices, heldItems, moves, stats, types,
				sprites);
		Pokemon charmeleon = new Pokemon(5, "charmeleon", count, count, false, count, count,
				new NamedApiResource("", "", 1), pokemonAbilities, results, gameIndices, heldItems, moves, stats, types,
				sprites);
		Pokemon charizard = new Pokemon(6, "charizard", count, count, false, count, count,
				new NamedApiResource("", "", 1), pokemonAbilities, results, gameIndices, heldItems, moves, stats, types,
				sprites);
		Pokemon blastoise = new Pokemon(9, "blastoise", count, count, false, count, count,
				new NamedApiResource("", "", 1), pokemonAbilities, results, gameIndices, heldItems, moves, stats, types,
				sprites);
		Pokemon butterfree = new Pokemon(12, "butterfree", count, count, false, count, count,
				new NamedApiResource("", "", 1), pokemonAbilities, results, gameIndices, heldItems, moves, stats, types,
				sprites);
		...
		when(pokemonMessageProducer.produceFindByName("bulbasaur"))
				.thenReturn(MessageBuilder.withPayload("bulbasaur").build());

	}

	@Test
	public void testFindByName() throws Exception {
		assertThat(pokemonService.findByName("")).isNotNull().isEmpty();
		assertThat(pokemonService.findByName(" ")).isNotNull().isEmpty();
		assertThat(pokemonService.findByName("x")).isNotNull().isEmpty();
		assertThat(pokemonService.findByName("aitor")).isNotNull().isEmpty();
		assertThat(pokemonService.findByName("bulbasaur")).isNotNull().isNotEmpty();
		assertThat(pokemonService.findByName("bulbasaur").get()).hasSize(1);
		assertThat(pokemonService.findByName("BuLbAsAuR")).isNotNull().isNotEmpty();
		assertThat(pokemonService.findByName("BuLbAsAuR").get()).hasSize(1);
		assertThat(pokemonService.findByName("char")).isNotNull().isNotEmpty();
		assertThat(pokemonService.findByName("char").get()).hasSize(3);
		assertThat(pokemonService.findByName("cHar")).isNotNull().isNotEmpty();
		assertThat(pokemonService.findByName("cHar").get()).hasSize(3);
		assertThat(pokemonService.findByName("b")).isNotNull().isNotEmpty();
		assertThat(pokemonService.findByName("b").get()).hasSize(3);
	}

	@Test
	public void testFindByNameAsync() {
		assertThat(pokemonService.findByNameAsync(null)).isNotNull().isEmpty();
		assertThat(pokemonService.findByNameAsync("")).isNotNull().isEmpty();
		assertThat(pokemonService.findByNameAsync(" ")).isNotNull().isEmpty();
		assertThat(pokemonService.findByNameAsync("aitor")).isNotNull().isNotEmpty();
		assertThat(pokemonService.findByNameAsync("aitor").get().getPayload()).isEqualTo("aitor");
		assertThat(pokemonService.findByNameAsync("bulbasaur")).isNotNull().isNotEmpty();
